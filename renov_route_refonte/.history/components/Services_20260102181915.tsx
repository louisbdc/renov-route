...
export default function Services() {
  ...
  useEffect(() => {
    if (displacementRef.current) {
      displacementRef.current.setAttribute('scale', isHovered ? '100' : '0')
    }
  }, [isHovered])
  ...
  return (
    ...
    >
      ...
      {/* Grid pattern visible avec distorsion */}
      <div 
      ...
      {/* Effet de "loupe" qui suit la souris - TRÈS VISIBLE */}
      <div 
        ...
        style={{
          ...
          width: isHovered ? '500px' : '0px',
          height: isHovered ? '500px' : '0px',
          background: `radial-gradient(circle, rgba(0, 240, 255, ${isHovered ? 0.25 : 0}) 0%, rgba(59, 130, 246, ${isHovered ? 0.15 : 0}) 30%, transparent 70%)`,
          borderRadius: '50%',
          filter: 'blur(60px)',
      ...
      {/* Indicateur visuel de test - à retirer après vérification */}
      {isHovered && (
        <div className="absolute top-4 right-4 bg-brand/80 text-white px-4 py-2 rounded-lg text-sm font-bold pointer-events-none z-20">
          DISTORSION ACTIVE
        </div>
      )}
