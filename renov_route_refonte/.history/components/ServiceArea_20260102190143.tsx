...
export default function ServiceArea() {
  ...
  useEffect(() => {
    const updateScrollProgress = () => {
      ...
      // Calculer quand le conteneur entre dans le viewport
      const containerTop = containerRect.top
      const containerBottom = containerRect.bottom
      
      // Commencer l'animation quand le conteneur entre dans le viewport (100% du viewport)
      // Finir l'animation plus tôt, quand le conteneur arrive à 50% du viewport
      const startPoint = windowHeight
      const endPoint = windowHeight * 0.5
      
      let progress = 0
      
      if (containerTop < startPoint && containerBottom > 0) {
        // Le conteneur est dans la zone d'animation
  ...
  // Calculer la hauteur de la barre animée en fonction de la hauteur réelle du conteneur
  const [containerHeight, setContainerHeight] = useState(200) // Valeur par défaut
  
  useEffect(() => {
    const updateHeight = () => {
      if (areasContainerRef.current) {
        // Utiliser scrollHeight pour obtenir la hauteur totale du conteneur
        // La barre doit aller du début du premier élément à la fin du dernier
        const height = areasContainerRef.current.scrollHeight
        setContainerHeight(height)
      }
    }
    
    // Utiliser un petit délai pour s'assurer que le DOM est rendu
  ...
  const animatedHeight = containerHeight * scrollProgress
    ...
    <section ref={sectionRef} className="py-24 bg-asphalt relative">
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="order-1 lg:order-2">
          ...
          <div ref={areasContainerRef} className="space-y-6 relative pl-4">
            {/* Barre de fond grise (fixe) */}
            <div 
              className="absolute left-0 top-0 w-1 rounded-full bg-gray-700"
              style={{
            ...
            <div 
              className="absolute left-0 top-0 w-1 bg-brand rounded-full transition-all duration-100 ease-out z-10"
              style={{
            ...
            {areas.map((area, index) => (
              <div key={index} className="flex items-start gap-4 relative z-20">
