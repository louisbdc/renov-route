...
// Modifier le manifest.json pour changer /contact par /qui-sommes-nous
function updateManifest() {
  try {
    ...
    if (fs.existsSync(manifestPath)) {
      ...
      // Modifier le raccourci "Nous contacter" pour pointer vers /qui-sommes-nous
      if (manifest.shortcuts && Array.isArray(manifest.shortcuts)) {
        const contactShortcut = manifest.shortcuts.find(shortcut => 
          shortcut.name === "Nous contacter" || shortcut.url === "/contact"
        );
        
        if (contactShortcut) {
          contactShortcut.url = "/qui-sommes-nous";
          contactShortcut.name = "Qui sommes-nous";
          contactShortcut.short_name = "À propos";
          contactShortcut.description = "Découvrez notre entreprise et notre expertise";
          
          // Réécrire le manifest modifié
          fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
          console.log('✅ Manifest.json mis à jour : /contact → /qui-sommes-nous');
        } else {
          console.log('⚠️  Raccourci "contact" non trouvé dans le manifest');
        }
      }
    } else {
      console.log('⚠️  Fichier manifest.json non trouvé dans /out/');
    }
  } catch (error) {
    console.log('❌ Erreur lors de la modification du manifest:', error.message);
  }
}

// Mettre à jour le manifest
console.log('📱 Mise à jour du manifest.json...');
updateManifest();
